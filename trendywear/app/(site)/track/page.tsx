...
function TrackPageInner() {
  ...
  const currentStep = statusMap[order?.status ?? "pending"] ?? 0;

  const progressSteps: ProgressStep[] = statusOrder.map((label, i) => ({
    label,
  ...
  const events: TrackingEvent[] = [
    { date: "Apr 12", time: "05:31", status: "Package awaiting pick up", active: true },
    { date: "Apr 12", time: "05:13", status: "Package awaiting shipping" },
    { date: "Apr 11", time: "17:37", status: "Order awaiting packaging" },
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F9FB] flex items-center justify-center">
        <p className="text-[#003049]/60 text-lg">Loading order...</p>
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#F8F9FB] flex items-center justify-center">
        <p className="text-[#003049]/60 text-lg">Order not found.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F8F9FB]">
      <main className="max-w-[1440px] mx-auto px-10 py-10">
        ...
        <section className="mt-3 mb-8">
          <h1 className="text-4xl font-bold text-[#C1121F] mb-2">Track Order</h1>
                ...
                <div key={step.label} className="flex items-center flex-1 min-w-0">
                  <div className="flex flex-col items-center min-w-[72px]">
                    ...
                    <p className={[
                      ...
                      step.completed || step.current ? "text-[#003049]" : "text-[#003049]/45",
                    ].join(" ")}>
                      {step.label}
                    </p>
                  </div>
                  {showLine && (
                    <div className="flex-1 h-[2px] mx-3 md:mx-4">
                      <div className={["h-full rounded-full", step.completed ? "bg-[#C1121F]" : "bg-gray-200"].join(" ")} />
                    </div>
                  )}
                </div>
              );
        ...
        <section className="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6 md:p-8 mb-8">
          <div className="mb-6">
            <p className="text-sm uppercase tracking-[0.14em] text-[#003049]/50 mb-2">Shipping Address</p>
            <p className="text-[16px] md:text-[18px] font-semibold text-[#003049]">
              Sinilyasi Street Block 57K Lot 12 Phase 3
            </p>
            <p className="text-sm md:text-base text-[#003049]/70">Caloocan City, Metro Manila 1400</p>
            <p className="text-sm text-[#003049]/60 mt-1">Jonas Cyrus Ponpon · 9171125164</p>
          </div>
          ...
          >
            ...
            <FiChevronDown className={`transition-transform duration-300 ${showDetails ? "rotate-180" : ""}`} />
          </button>

          <div className={`overflow-hidden transition-all duration-300 ease-out ${showDetails ? "max-h-[900px] mt-5" : "max-h-0"}`}>
        ...
        <section className="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6 md:p-8 mb-10">
          <div className="rounded-[22px] bg-[#FAFAFA] border border-gray-200 px-5 py-4 mb-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/45">Order No.</p>
                <p className="mt-1 text-sm font-semibold text-[#003049]">{order.id}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/45">Payment</p>
                <p className="mt-1 text-sm font-semibold text-[#003049]">GCash</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/45">Items</p>
                <p className="mt-1 text-sm font-semibold text-[#003049]">{totalItems} item{totalItems > 1 ? "s" : ""}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/45">Current Status</p>
                <p className="mt-1 text-sm font-semibold text-[#2A9D8F] capitalize">{order.status}</p>
              </div>
            </div>
          </div>

          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[22px] font-semibold text-[#003049]">Products ({totalItems} items)</h2>
            <div className="text-right">
              <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/50">Subtotal</p>
              <p className="text-lg font-semibold text-[#C1121F]">PHP {subtotal.toLocaleString()}.00</p>
            </div>
          </div>
...
function Timeline({ events, highlightUntil = 0 }: { events: TrackingEvent[]; highlightUntil?: number }) {
  return (
    <div className="space-y-0">
      {events.map((event, index) => {
        ...
        return (
          <div key={`${event.date}-${event.time}-${index}`} className="flex gap-4">
            <div className="w-[68px] shrink-0 pt-0.5 text-right">
              <p className={`text-sm font-semibold ${isHighlighted ? "text-[#2A9D8F]" : "text-[#003049]/55"}`}>{event.date}</p>
              <p className="text-[11px] text-[#003049]/40">{event.time}</p>
            </div>
            <div className="relative flex flex-col items-center">
              <span className={`w-3 h-3 rounded-full border-2 z-10 ${isHighlighted ? "bg-[#2A9D8F] border-[#2A9D8F]" : "bg-white border-[#D7D7D7]"}`} />
              {!isLast && <span className={`w-[2px] h-[54px] mt-1 ${isHighlighted ? "bg-[#2A9D8F]" : "bg-[#E5E5E5]"}`} />}
            </div>
            <div className="pb-5">
              <p className={`text-sm md:text-[15px] ${isHighlighted ? "text-[#003049] font-medium" : "text-[#003049]/55"}`}>{event.status}</p>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="py-5 flex items-center justify-between gap-6">
      ...
      <div className="text-right shrink-0">
        <p className="text-xs uppercase tracking-[0.14em] text-[#003049]/45">Item Total</p>
        <p className="mt-1 text-[18px] font-semibold text-[#C1121F]">PHP {(item.price * item.quantity).toLocaleString()}.00</p>
      </div>
    </div>
  );
}
